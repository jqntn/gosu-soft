...
S1(float v)
{
  return sinf(0.5f + v * 10.0f) * 0.1f;
}

float
S2(float v)
{
  return sinf(v * 10.0f) * 0.1f;
}
...
P(float x, float y)
{
  ...
  float X = fmodf(s * x, 1.0f);
  float Y = fmodf(s * y, 1.0f);

  int i1 =
    (int)fmodf(floorf(s * x), s) + s * (int)fmodf(floorf(s * y), s) + 1.0f;
  int i2 = (int)fmodf(floorf(s * x) + 1.0f, s) +
           s * (int)fmodf(floorf(s * y), s) + 1.0f;
  int i3 = (int)fmodf(floorf(s * x), s) +
           s * (int)fmodf(floorf(s * y) + 1.0f, s) + 1.0f;
  int i4 = (int)fmodf(floorf(s * x) + 1.0f, s) +
           s * (int)fmodf(floorf(s * y) + 1.0f, s) + 1.0f;

  float d1 = X * cosf(g[i1]) + Y * sinf(g[i1]);
  float d2 = (X - 1.0f) * cosf(g[i2]) + Y * sinf(g[i2]);
  float d3 = X * cosf(g[i3]) + (Y - 1.0f) * sinf(g[i3]);
  float d4 = (X - 1.0f) * cosf(g[i4]) + (Y - 1.0f) * sinf(g[i4]);
...
fs(float* fs_in, Shader_Builtins* builtins, void* uniforms)
{
  ...
  static const vec4 pastelRed = { 0.78f, 0.27f, 0.25f, 1.0f };
  static const vec4 lightPurple = { 0.49f, 0.43f, 0.70f, 1.0f };
  static const vec4 softPurple = { 0.40f, 0.37f, 0.58f, 1.0f };
  static const vec4 darkPurple = { 0.05f, 0.05f, 0.13f, 1.0f };

  custom_uniforms* p_uniforms = ((custom_uniforms*)uniforms);

  vec4 color = lightPurple;

  float x = (builtins->gl_FragCoord.x - 0.5f) / p_uniforms->width;
  float y = (builtins->gl_FragCoord.y - 0.5f) / p_uniforms->height;
  ...
  bool smoke = 0.25f * P(x1 - a, y - a) + (x1 - 0.5f) * (x1 - 0.5f) +
                 (y2 - 0.5f) * (y2 - 0.5f) <
               0.14f;
  if (smoke) {
    color = softPurple;
  ...
  bool body = (x1 - 0.47f) * (x1 - 0.47f) + (y1 - 0.48f) * (y1 - 0.48f) < 0.06f;
  if (body) {
    color = darkPurple;
  }

  bool leftEye = ((x1 - 0.3f) * (x1 - 0.3f)) / (0.6f * 0.6f) +
                     ((y1 - 0.54f) * (y1 - 0.54f)) <
                   0.022f &&
  ...
  bool rightEye = ((x1 - 0.54f) * (x1 - 0.54f)) / (0.9f * 0.9f) +
  ...
  bool leftPupil = ((x1 - 0.305f) * (x1 - 0.305f)) / (1.0f * 1.0f) +
  ...
  bool rightPupil = ((x1 - 0.516f) * (x1 - 0.516f)) / (1.0f * 1.0f) +
                      ((y1 - 0.48f) * (y1 - 0.48f)) / (4.0f * 4.0f) <
                    max(0.00002f, S2(a) * 0.0004f);
  if (rightPupil) {
    color = darkPurple;
  ...
  bool mouth = ((x1 - 0.45f) * (x1 - 0.45f)) / (1.1f * 1.1f) +
                   ((y1 - 0.49f) * (y1 - 0.49f)) / (1.0f * 1.0f) <
                 0.04f &&
               ((x1 - 0.45f) * (x1 - 0.45f)) / (1.5f * 1.5f) +
                   ((y1 - 0.54f) * (y1 - 0.54f)) / (1.0f * 1.0f) >
                 0.04f &&
               y1 - 0.32f < fabsf(3.0f * x1 - 0.91f) &&
               y1 - 0.29f < fabsf(2.5f * x1 - 1.4f);
  if (mouth) {
    color = pastelRed;
  }

  bool leftTooth = y1 - 0.32f > fabsf(3.0f * x1 - 0.91f) &&
                   ((x1 - 0.45f) * (x1 - 0.45f)) / (1.5f * 1.5f) +
                       ((y1 - 0.54f) * (y1 - 0.54f)) / (1.0f * 1.0f) >
                     0.04f &&
                   y1 < 0.5f;
  ...
  bool rightTooth = y1 - 0.29f > fabsf(2.5f * x1 - 1.4f) &&
                    ((x1 - 0.45f) * (x1 - 0.45f)) / (1.5f * 1.5f) +
                        ((y1 - 0.54f) * (y1 - 0.54f)) / (1.0f * 1.0f) >
                      0.04f &&
                    y1 < 0.5f;
